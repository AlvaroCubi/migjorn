...
`migjorn` and `ipykernel`. From this directory (`crates/migjorn-py`):
...
nbconvert) into `crates/migjorn-py/.venv`. Then, in your IDE, select the kernel /
interpreter:
...
> get no autocomplete/typing. A non-editable install places `migjorn/__init__.pyi`
...
`migjorn._migjorn` submodule, wrapped by the `migjorn` package which ships type
stubs (`python/migjorn/__init__.pyi`) and a `py.typed` marker. Pylance, pyright,
...
`src/lib.rs`, update `python/migjorn/__init__.pyi` to match.
