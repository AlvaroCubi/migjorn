...
# ///
"""A guided tour of migjorn, runnable with a single command:

    uv run crates/migjorn-py/examples/migjorn_demo.py

uv reads the inline metadata above, builds the local `migjorn` package
...
For the interactive notebook version, see `migjorn_demo.ipynb` and the uv
...
MODEL = """\
Migjorn demo: a small pin-cell-ish model
c --- cells ---
