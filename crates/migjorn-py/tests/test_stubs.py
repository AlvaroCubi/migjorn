...
Parses the shipped stub (`migjorn/__init__.pyi`) and compares its public API to
...
def _load_stub() -> ast.Module:
    pyi = os.path.join(os.path.dirname(migjorn.__file__), "__init__.pyi")
...
def _runtime_module_api() -> set:
    api = set()
    for n in dir(migjorn):
        if n.startswith("_"):
            continue
        obj = getattr(migjorn, n)
...
def test_class_members_match_stub():
    ...
    for cls in PUBLIC_CLASSES:
        runtime = _public(dir(getattr(migjorn, cls)))
