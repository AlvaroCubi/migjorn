...
Model
 ├─ cst: Cst
 │    ├─ arena: Vec<Option<Card>>    // index == slot; append-only, never reordered
 │    ├─ order: Vec<u32>             // file order, as slots
 │    └─ eol:   Eol                  // this file's terminator, for added cards
...
self-contained, and the original source is not retained after parse.

> **Amended during implementation: arena + order, not a flat `Vec<Card>`.**
> A single `Vec<Card>` cannot deliver all three of the properties this design
> needs at once. Resolving a stable slot to a position would need a side table
> that every insert has to repair — a scattered write per card, on top of a
> 56-bytes-per-card memmove. Splitting into an append-only `arena` (index =
> slot) plus an `order` vector of slots gives:
>
> * `slot → card` is a **direct index**: O(1), and never invalidated.
> * insert/remove moves **4 bytes per card** in `order`, with no side table.
> * emission stays sequential, because `arena` starts in file order and only
>   user-added cards are ever out of place.
> * removal tombstones the slot (freeing its text) so it can never be reused.
>
> Finding a slot's *position* is a linear scan of `order` — 4 bytes per card,
> sequential and cache-friendly (~0.3 ms per million cards), which is cheaper
> than maintaining the side table it replaces.
...
Card {
  slot:        u32,
  kind:        CardKind,
  has_unknown: bool,       // set at lex time; see "Tokens index the card"
  text:        String,     // this card's exact current bytes
  tokens:      Vec<Token>, // tokens indexing `text` (spans relative to `text`)
}
```
...
current bytes and the `Vec<Token>` indexing it. Consequences:
...
### Tokens index the card; they do *not* tile it
> **Amended during implementation.** The original plan carried over the current
> design's "tokens tile the source, trivia included" rule. That rule exists to
> make losslessness a concatenation *of tokens* — but in this design
> losslessness comes from `card.text`, which already holds the card's exact
> bytes. Emission never consults a token. So tiling buys nothing, and we drop it.

Within a card, tokens cover only what a reader needs to address: numbers,
identifiers, operators, `$`/`c` comments, and `&`. **Whitespace and newlines
produce no token at all** — the gap between two spans *is* the whitespace, and
the exact bytes are recovered from `card.text`.

`Token { kind, start: u32, len: u32 }`. Measured on `big.mcnp`: **62.4M tokens**
where the tiling scheme produced 116.6M — a little under half the token memory,
and a correspondingly cheaper parse. Comments and `&` are kept as tokens because
the parser must skip them explicitly.

One consequence worth stating: a consumer scanning for malformed cards must not
walk every token of every card to find `Unknown`s — on a million cards that is
tens of millions of scattered reads, and it measurably dominated index building.
`Card` therefore carries a `has_unknown` flag, set at lex time (when the tokens
are cache-hot) and stored in padding the struct already had.
...
- **Per-card memmove on insert/remove is O(num_cards).** For ~1M cards a memmove
  ...
  API that does one memmove instead of one per card. (Do not prematurely switch
  to a gap buffer / linked structure; measure first.)

  Implemented as `Cst::insert_many_at` (one `Vec::splice` for the whole batch,
  O(n + m) instead of O(n·m) for `m` inserts into an `n`-card model) once a
  downstream consumer's whole-project `Model::merge` — which folds another
  model's cells/surfaces/data in one card at a time — hit exactly this
  quadratic blowup. `merge` now groups incoming cards by kind and absorbs each
  block through `insert_many_at` (`compose.rs`'s `absorb_many`) instead of
  calling the single-card `insert_at` per card.
