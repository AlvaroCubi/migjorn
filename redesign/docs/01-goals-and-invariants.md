...
API shape and the domain knowledge of the current library (both are good), but
replaces the core data structure, which was built for a parse-renumber-emit
batch pipeline and does not support cheap structural editing (see
[`02-architecture.md`](02-architecture.md) for the measured evidence).
...
- Full modelling of every exotic data card's grammar is **out of scope**: data
